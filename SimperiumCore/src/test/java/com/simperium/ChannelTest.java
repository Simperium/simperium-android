...
import com.simperium.test.util.ChannelUtil;
import com.simperium.test.util.RemoteChangesUtil;
...
import static com.simperium.TestHelpers.waitFor;
...
public class ChannelTest extends TestCase {
    ...
    throws Exception {
        ...
        String message = mListener.lastMessage.toString();
        assertEquals("c:{", message.substring(0,3));
        assertEquals("}", message.substring(message.length()-1, message.length()));
    ...
    throws Exception {
        ...
        */
        ...
        assertEquals("Index: " + index, expected.getJSONArray("index").length(), index.getJSONArray("index").length());
